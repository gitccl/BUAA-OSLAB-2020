...
// 	if(syscall_mem_alloc(0, diskaddr(blockno), PTE_V|PTE_R) < 0){
// 		user_panic("Error in fs.c/map_block. mem_alloc failed\n");
// 	}
...
unmap_block(u_int blockno)
{
	...
	if(r < 0) {
		user_panic("Error in fs.c/unmap_block mem_unmap\n");
	} 
...
free_block(u_int blockno)
{
    ...
    if(blockno == 0 || super == 0 || blockno >= super->s_nblocks ){
		user_panic("Error in fs.c/free_block: [invalid blockno] \n");
	}
...
dir_lookup(struct File *dir, char *name, struct File **file)
{
	...
	for (i = 0; i < nblock; i++) {
        ...
        /* blk是第i个文件块对应的磁盘块的虚拟地址, 磁盘块的内容已经读取到该地址上了 */
		if( (r = file_get_block(dir, i, &blk)) < 0) {
			writef("Error in fs.c/dir_lookup: file_get_block\n");
			return r;
		}
